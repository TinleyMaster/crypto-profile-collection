...
**⚠️ 交付状态：代码未提交（与并发进程 WIP 交织）**。本轮代码改动（`scan_daemon.py` / `check_scan_freshness.py`）与并发进程（workbuddy / commit 作者 `fai婷酱`）的 **`breakout_px`**（设计文档 v0.5「`active→confirmed` 延续确认」）未提交改动**落在同一批 hunk**（主池 `trig_px/stop_pct` 与 `brk_px` 同块、`task_scan_accumulation` 的 INSERT 同块）⇒ 按「只提交自己改的文件、不纳入并发进程改动」的约定，本轮**未提交**，仅记录设计决策与物证。收口方式见本节末「交接」。
...
- **P0-1（P0，已改未提交）`expire_signals` 常驻首轮几乎永不触发**：`_run_task_loop` 语义是「先 `sleep(offset)` 再循环」，offset 即**首轮最早时刻**；而容器重启周期实测 **493/593/719/911 秒（约 15 分钟）**，`expire_signals` 的 offset 恰为 **900s** ⇒ 与进程存活时长同量级，每次重启都把首轮推到期前。**连带**：两条停摆监控（daemon `_stall_parts` + 看门狗 `check_scan_freshness`）对「本实例尚未跑完首轮」一律给 **3×周期** 宽限（1800s→90min / 86400s→4320min），远大于 15 分钟重启周期 ⇒ 每次重启刷新宽限，**「线程从未启动」被无限期掩盖**（两条监控同时静默）。
  ...
  - 物证（只读 prod，09-22 08:11 CST）：`biz.scan_heartbeat` 中 `expire_signals` 曾**长期 0 行**（表 10 行 = 9 任务 + `__daemon__`），本次探测已变为 **1 行 / 11 任务**（说明有过一次成功），但 `status='active'` 仍积到 **894 行、最老 09-16 09:30 UTC**，`expired` 仅 132 行 —— 生命周期「超时退出」事实上仍靠 `--run-once` 驱动。
- **P2-4（P2，已改未提交）主池失效位幅度不可用**：原「近 `LOOKBACK_BARS_MAIN+1`=21 根反向极值」实测**风险回报倒挂**（XMR -11.97% / EPIC -8.02% / 1000BONK -8.13% / APT -6.90%，其中一例自身涨幅仅 +4.69% 却配 -11.97% 止损），窄幅盘整时又会紧到 0.3% 被噪音打掉。改为 **`2×ATR(14)` / 参考价，夹在 `[3%, 12%]`**（常量 `STOP_ATR_MULT=2.0` / `STOP_ATR_PERIOD=14` / `STOP_PCT_MIN=3.0` / `STOP_PCT_MAX=12.0`）；K 线不足 `PERIOD+1` 根返回 **None 不兜底**（与费率/共振的 `n/a` 同口径）。物证：prod 主池 `stop_loss_pct` 分布 min 5.64 / 中位 9.89 / **max 21.18**，其中 **>10% 有 5 行**。
