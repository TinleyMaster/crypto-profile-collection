...
- **静默是「两容器同停同起」，不是单容器崩溃**：主容器探针 `biz.kol_post`（kol_daemon 每 30s）与**美区节点独立容器**探针 `biz.asset_klines.fetched_at`（scan_daemon 5min 任务）**同窗口静默**——US 侧末次写入 **09-25 14:13:41**、首次恢复 **09-26 13:25:43**（逐小时计数：09-25 14 之后整段为 0，直到 09-26 13 才 2162）；主容器探针同分钟恢复（13:23~13:24）。该 23h 静默**跨两个节点的两个容器同时发生** ⇒ 指向**平台级事件**（容器被 pause/驱逐 或 网络分区），而非单容器 OOM。
- **窗口内并非全库零写入**：`[看护]` 心跳在 09-25 16:19 / 09-26 05:41 / 12:36 有**孤立成功写入** ⇒ 更符合「容器被平台反复 pause/resume、每次恢复首轮写一条心跳」，而非恒定写不可达。
