...
- **B 组待用户授权（本轮不代跑）**：**B1 重算 PONS thesis**（写 `biz.research_thesis`）、**B2 tokenomist 重抓**。二者均为**写 prod DB / 触外部采集**，遵守「AI 不连 prod DB 写」硬约束，**须用户显式授权后再执行**，本轮不动。
...
- **未做 / 边界**：① 工单 A1②/A2/A4 未重做（已由 `5ef3f02` 落地）；② A2 `kind` 保留 `fdv_degenerate`（工单 `degenerate_fdv` 同义）；③ B1/B2 待授权；④ 线上 runtime 复验（重拉 notebook / market-history / overview）须待 Zeabur 重建后执行（`push ≠ 线上生效`）。
