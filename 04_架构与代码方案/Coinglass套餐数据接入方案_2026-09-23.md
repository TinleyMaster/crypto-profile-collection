...
- **P0** 让「已采未用」的列进入消费，并把爆仓比率改成**口径自洽**的算法（修 §10.5 混合口径）；
...
2. **口径缺陷**（P0-C）：§10.5 已披露「分子 = CoinGlass **全交易所**爆仓额 / 分母 = **Binance** 24h 成交额」⇒ 比率系统性偏高，两个 `*_LIQ_RATIO` 阈值是在混合口径下标出来的临时值。而套餐里**恰好有单交易所口径**（`liquidation/history` 的 `exchange=Binance`），能一次性把口径拉齐。
3. **能力缺口**（P1）：`liquidation_snapshot` 是**滚动窗口快照**、自 09-16 起才积累，导致 §12.1 的 A3（无样本外）、B11（爆仓阈值只有临时值）、C9（爆仓维历史≈0）**无法用数据解决**——而 `aggregated-history` 提供的是**分段增量**，180 天 × 4h 可直接当回测正样本。
...
- G2：爆仓比率**同源同窗口**，为阈值重新标定提供合法依据（不产出数字结论，只产出可标定样本）。
...
| 历史范围 @4h | **180 天** | @6h/8h/12h = 360 天；@1d = 全历史 |
...
8. **符号映射精确码优先**（§10.4）：复用 `sqz.alias_bases()` / `_perp_alias_map()`，防 `1000PEPEUSDT` 抢 `PEPE`。
...
| 项 | 估算 |
|---|---|
| 每币每口径请求数 | 180 天 ÷ 4h = **1080 点**，limit 上限 1000 ⇒ **2 次** |
| 总请求数 | 527 × 2（口径）= **1054 次**（仅 Binance 口径） → 两口径 **2108 次** |
...
- **可续跑**（强约束，非可选）：本项目容器重启是常态（AGENTS.md 记录多次停摆），88 分钟的作业必然被中断 ⇒ 必须有游标表（§5 的 `biz.liquidation_backfill_cursor`），`--resume` 从断点继续。
...
CREATE TABLE IF NOT EXISTS biz.liquidation_history (
    symbol          TEXT        NOT NULL,        -- 币种码，如 BTC（与 coin-list 的 symbol 同口径）
...
| `--symbols BTCUSDT,ETHUSDT` | 指定币（默认全池，映射走 `alias_bases()` 精确码优先） |
...
- 现役 `SQZ_SHORT_LIQ_RATIO_MIN` / `LONG_LIQ_RATIO_THR` **保持不动**，直到口径 B 给出合法基准。
...
# 7) 标定脚本只读复跑（P0-C 之后）：期望口径 A/B 分表输出，缺样本时 rc!=0
...
| **容器重启中断回填** | 本项目重启是常态（AGENTS.md 多次停摆记录） | 游标表 + `--resume`（强约束） |
...
| **符号映射错配** | `1000PEPE` / `SHIB` 等前缀币 | 复用 `alias_bases()` + `setdefault` 精确码优先；小样本验收含前缀币 |
...
| 同上 §10.5 口径披露 | 增列口径 B（同源同窗口 4h），并明确 A/B 不可换算 | ⬜ 随 P0-C |
