-- ============================================================
-- 催化剂错连资产脏数据清理脚本（全量版）
-- 背景：linker.py 旧代码 symbol→asset_id 兜底无市值排序，
--       导致 BTC/XRP/BNB 等主流币被映射到同名小市值仿盘
--       （Bitcoin Base / XRP AI / BNBTiger Inu 等）
-- 修复逻辑：
--   1. 找出所有「同 symbol 中市值非最大」的错连资产
--   2. 删除这些资产在 catalyst 相关表中的脏记录
--   3. 保留每 symbol 市值最大的那条资产关联
...
-- ---- Step 1: 识别所有"同 symbol 中非市值最大"的错连资产
-- （这些是 linker 旧逻辑可能错选的资产）
...
WITH ranked AS (
    SELECT asset_id, canonical_symbol, canonical_name, market_cap,
           ROW_NUMBER() OVER (
               ...
               ORDER BY COALESCE(market_cap, 0) DESC, asset_id
           ) AS rn
    FROM core.asset
...
FROM ranked
WHERE rn > 1
  AND market_cap IS NOT NULL  -- 只清理有市值数据的（明确有更大市值真币的）
ORDER BY UPPER(canonical_symbol), market_cap DESC;
...
-- （可选）查看错连资产明细（前 30 条）
-- SELECT canonical_symbol, canonical_name, market_cap, asset_id
-- FROM wrong_assets
