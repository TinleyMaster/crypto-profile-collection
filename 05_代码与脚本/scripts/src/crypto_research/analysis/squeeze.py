...
# 2026-09-21 全样本（8018 行 / 211 币）：P75=6.09e-5 / P90=2.27e-4 / P95=4.07e-4；
# 旧值 8e-5 落在 P75 附近，且「冲高回撤」条件子集（n=1202）越阈率高达 43.2% →
# 21%~43% 的采样被标成 big_long_liq，**系统性屏蔽 profit_take / long_win**，
# 结论被推向 churn/short_win。故上调至无条件 P95（回撤子集越阈率降至 14.6%<20%，
# 满足工单 SQZ-02 的「不因市场状态高频而回退 P90」判据）。
# ⚠️ 临时值：待 squeeze_track 判定样本积累（条件分布直接算在判定窗口上）后定稿。
