...
INGEST_SCRIPT = SCRIPT_DIR / "ingest_cmc_info.py"


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run ingest_cmc_info.py in repeated batches using --from-map-missing."
    )
...
def main() -> int:
    ...
    for batch_no in range(1, args.max_batches + 1):
        ...
        if args.dry_run:
            break
        if payload and payload.get("status") == "noop":
            break
        if payload and payload.get("row_count") == 0:
            break
