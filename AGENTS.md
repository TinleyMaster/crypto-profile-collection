...
- **SQZ-02（P1，已改）`LONG_LIQ_RATIO_THR` 0.00008 → 0.00040709**：
  ...
  - 采 **无条件 P95 量级 = 4.07e-4**（条件子集越阈率降至 ≈15%~18%）；`SQZ_SHORT_LIQ_RATIO_MIN` 保持 0.00008（≈全样本 P90，位置合理）；两者确认独立常量、不联动。
...
- **P2-d（🟠 已修）判定窗口「中段空洞」双判据都拦不住**：覆盖率只看**数量**（13 桶缺 2 个中间桶 → `11/13=0.846` 通过）、`tail_gap` 只看**右端** ⇒「前段齐、中段缺、尾部齐」无人管，而重启/`os._exit(1)` 杀的恰恰是**中间某桶**（快照不可回补）。已在 `scan_daemon.py` 新增 `MAX_MID_GAP_BUCKETS = 2`：统计 `[first_bucket, last_bucket-1]`（右端正在采集的桶归 `tail_gap` 管）内最长连续缺桶，≥2 即拒判，reason=`判定窗口中段连续缺桶 N 个…`（仍以 `判定窗口` 开头 ⇒ 看门狗的 `reason LIKE '判定窗口%'` 计数自动涵盖），metrics 增记 `mid_gap_buckets`。
...
- **⚠️ 本轮重大副产品（样本仍在漂移，供拍板）**：脚本首次运行显示**结论对取样时段极度敏感**——同一公式，5.92h 样本（09-21 02:00~08:00 UTC，亚洲清淡时段）得无条件越阈率 ≈20%、入队侧 short ≈11%；22.75h 样本（含美盘活跃时段）得 **无条件 40.87% / 入队侧 short 35.10% / 条件子集 30.8%~34.7%**。⇒ **「无条件分位」标定方式本身不稳健**（把不同市场状态混在一起取分位），正解是按**判定窗口的条件分布**标定（复验 §9-1/§9-6 已建议）。**未改阈值**：等用户拍板（回退 P90 / 按条件分布重标 / 先用当前值）。
...
**⑥ 【P1 待修】主池 `_l1_screen` 对「后半段才完成的异动」系统性漏检**（本轮新发现，`scan_daemon.py:696-726`）
...
- **建议修法（未实施，待拍板；改动会提升信号量，勿顺手改）**：把 L1 的触发根由「只看 `closes[-1]`」改为 **「优先看最近一根**已收盘**条（值稳定 ⇒ 判定可复现），不合格再回退未收盘条（保持及时性）」**。去重**无需新增机制**——现有 `_in_cooldown_main`（6h）天然拦住「未收盘先入池、收盘又命中」的重复。副作用：① 入场/失效位（`trigger_price`/`breakout_px`/`stop_loss_pct`）必须显式锚定到**被判定的那一根**，不能继续写死 `bars[-1]`；② 需同步 `backtest_scan_scenarios.py` 的 L1 口径，否则线上与回测不一致（与 agap P2-3「oi_dir 二值」同类的口径对齐要求）。