...
- **未做**：① `fix_070` 之外的存量脏数据未清（`no_price=43` 属日价源缺口，非本刀范围）；② 回测**未接调度**（按用户决策「先手动跑通」，故 `biz.signal_type_calibration` 目前为一次性快照，需人工重跑 `backtest_opportunities.py --days 30 --write` 刷新）；③ 前端 HIGH 计数复验待部署后补。
