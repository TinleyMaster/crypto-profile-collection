...
def map_pairs_to_asset_ids(
    ...
    for pair in pairs:
        ...
        # （同 symbol 多行时避免取到脏行：
        #   1. 排除 tokenized/合成/商品衍生/AI 仿盘/桥接/包装/meme 仿盘 等脏标记
        #   2. 按 CMC 排名升序（rank 越小越主流），兜底用 asset_id
        #   3. 语义排序仅作次级参考（名称含 symbol > 普通 > 贵金属商品）
        ...
        with conn.cursor(row_factory=psycopg.rows.dict_row) as cur:
            cur.execute(
                ...
                WHERE UPPER(canonical_symbol) = %s
                  ...
                  AND LOWER(COALESCE(canonical_name, '')) !~ '\bai\b|second[[:space:]]+chance|tiger[[:space:]]+inu|\binu\b|base[[:space:]]+coin|gold[[:space:]]+ai|bridged|wrapped|intents|trophy|tomato|meme|doge|shib|pepe|floki|babydoge'
                ORDER BY
