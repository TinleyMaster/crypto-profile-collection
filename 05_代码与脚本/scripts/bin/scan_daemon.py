...
def _others_clause(kinds: list[str], *exclude: str) -> str:
    """标题「另有币…」并列子句（N-62B-1）。

    排除已由主句陈述的形态（`exclude`），避免同一批单币时出现「无催化剂条目，另有币无催化剂」
    这类同义重复 —— 主句与并列句必须描述**不同**的币。
    """
    rest = [k for k in kinds if k not in exclude]
    return f"，另有币{'、'.join(rest)}" if rest else ""
...
def _alert_title(items: list[dict]) -> str:
    ...
    has_zero_cat = False  # **已关联**且催化剂真为 0 条（N-786-4：不得误述「全部 >3 天」）
    has_unlinked = False  # N-62B-1：资产未关联 ⇒ 催化剂/KOL 段**无从查询**（n/a ≠ 0）
    ...
    for it in items:
        ...
        if ctot > 0 and f_coin == 0:
            has_stale = True
        # N-62B-1：卡片侧已确立「0 ≠ n/a」（L2006/L2048 对未关联渲染 n/a）⇒ 标题侧
        # 也必须区分：未关联既不算「有陈旧催化剂」，也不算「催化剂真为 0 条」。
        if not res.get("asset_linked", True):
            has_unlinked = True
        elif ctot == 0:
            has_zero_cat = True
    # N-62B-1：本批并存的「非新鲜方向」形态（零催化剂 / 未关联），两条返回路径共用
    # —— 旧实现只看 `has_zero_cat`，混合批次会把未关联币一起说成「无催化剂条目」。
    kinds = []
    if has_zero_cat:
        kinds.append("无催化剂")
    if has_unlinked:
    ...
    if not n_res:
        ...
        # 「本批还同时含零催化剂币」；实测近 7 天 6 批真实触发（最强 `09-23 00:32:30`
        # 的 4 币批里 3 币零催化剂，标题却断言「催化剂**全部** >3 天」）⇒ 与非密封侧同族措辞。
        # N-2B4D-1：本支文案核与图例、非密封侧**逐字一致**（「含仅陈旧条目，另有币无催化剂」）。
        if has_stale:
            if kinds:
                return (f"🚨 盘面异动告警：{len(items)} 币高置信信号"
                        f"（无新鲜共振；含仅陈旧条目{_others_clause(kinds)}）")
            return (f"🚨 盘面异动告警：{len(items)} 币高置信信号"
                    f"（无新鲜共振；催化剂全部 >{CATALYST_STALE_DAYS} 天，不计方向）")
        if has_unlinked:
            # N-62B-1（弱形式，线上 4/123 批可达）：未关联币的催化剂/KOL 栏渲染 n/a ⇒
            # 标题不得断言「无共振」（对它是无从查询，不是事实为空）。已关联且确为 0 条
            # 的币不在此列（那仍是事实空集）⇒ 保持 S1 原文案，把改动面压到最小。
            return (f"🚨 盘面异动告警：{len(items)} 币高置信信号"
                    f"（纯盘面信号；未关联资产，催化剂 n/a"
                    f"{_others_clause(kinds, '未关联资产')}）")
    ...
    # 审计 N-923-1：单币批次的「（1/1 币）」与条数重复 ⇒ 仅多币时渲染，且方向段用
    # 「新鲜」**前缀**而非「（已剔除陈旧）」**后缀** —— 后者在无陈旧可剔时（实测
    # 46% 共振币）是虚假暗示，且「新鲜」二字与图例「方向段用新鲜口径」天然一致。
    cover_txt = (f"（{all_coins}/{len(items)} 币）" if len(items) > 1 else "")
    dir_txt = ""
    # N-0F7-1：本支原为**唯一**不读 has_zero_cat / has_unlinked 的返回路径 —— 同封邮件里
    # 卡片对未关联币渲「催化剂n/a」、对已关联真 0 条币渲「催化剂0」，标题却把两者一并并进
    # 单向度计数串（实测 09-22 11:47 的 4 币批：1 新鲜 + 1 未关联 + 2 真 0 ⇒ 标题只印
    # 「催化剂新鲜 0多/0空/1中」，读者会把 4 币全读成已覆盖）⇒ 与其余五支同族补并列子句。
    if fresh_bull or fresh_bear or fresh_neut:
        dir_txt = f"，催化剂新鲜 {fresh_bull}多/{fresh_bear}空/{fresh_neut}中"
        # 审计 O2：中性条不计入方向。只报「多/空/中」时，扫读者会把「4多/0空/2中」
        # 读成强多；补「净多 = 多−空」，让中性不增强 direction conviction。
        if fresh_bull != fresh_bear:
            dir_txt += (f"，净{'多' if fresh_bull > fresh_bear else '空'}"
                        f"{abs(fresh_bull - fresh_bear)}")
        dir_txt += _others_clause(kinds)
    # N-416-1：非「有新鲜方向」时按**逐币事实**三分支——旧实现用全局布尔优先级，
    # 混合批次（既有零催化剂币、又有全陈旧币）会二选一而把另一侧说错
    #（`has_zero_cat` 抢先 ⇒ 全陈旧币被误述「无催化剂条目」，反之同理）。
    # N-62B-1：补第 4 分支（未关联）——未关联币的催化剂/KOL 栏渲染 n/a，若此处无分支
    # 会**落空档**（n_res 由事件段供、催化剂无从查询，标题既不报口径也不报 n/a）。
    elif has_stale and has_zero_cat:
        dir_txt = f"，催化剂新鲜条目 0（含仅陈旧条目{_others_clause(kinds)}）"
    elif has_stale:
        # N-786-3：全陈旧 ⇒ 新鲜方向根本不存在，不得说成「多空持平」（含义相反）。
        # N-62B-1：若同批另有未关联币，「全部 >X 天」对它不成立 ⇒ 退一级用「含」措辞。
        _base = (f"全部 >{CATALYST_STALE_DAYS} 天，不计方向" if not has_unlinked
                 else "含仅陈旧条目")
        dir_txt = f"，催化剂新鲜条目 0（{_base}{_others_clause(kinds)}）"
    elif has_zero_cat:
        # N-786-4：本批存在「催化剂真为 0 条」的币（n_res 由 event/KOL 贡献）——不得
        # 断言「全部 >3 天」（卡片会显示「催化剂0」，同封邮件自相矛盾）。
        dir_txt = f"，催化剂新鲜条目 0（无催化剂条目{_others_clause(kinds, '无催化剂')}）"
    elif has_unlinked:
        # N-62B-1：该仓位未关联 core.asset ⇒ 卡片为「催化剂n/a · KOL n/a」，标题不得
        # 断言「无催化剂条目」（那是 0，与 n/a 语义相反）。
        dir_txt = "，催化剂 n/a（未关联资产，非 0）"
...
def _render_alert_email(items: list[dict],
    ...
    legend = ("<p style='color:#6b7280;font-size:12px'>图例：场景编号按行自身维度重算 —— "
              ...
              "（与卡片「催化剂 N」同一口径；单币批次省略「（k/M 币）」——「1/1 币」与条数"
              "重复），其方向段以「催化剂新鲜 X多/…」标口径"
              "（陈旧不计；避免无陈旧可剔时仍印「已剔除陈旧」的虚假暗示；该段与其余各段一样"
              "可并列「另有币…」子句，见下）、"
              "方向段「无催化剂条目」= 该仓位无任何催化剂（n_res 由事件/KOL 贡献）、"
              "「全部 >X 天」= 催化剂全部陈旧、「含仅陈旧条目，另有币无催化剂」= 本批两者皆有、"
              "「未关联资产」= 该币未关联 core.asset（催化剂/KOL 栏为 n/a，无从查询 ≠ 0）；"
              "整批共振为 0 时标题取「纯盘面信号」或「无新鲜共振」前缀"
              "（前者 = 各币共振段皆空、后者 = 存在催化剂仅陈旧的币），"
              "并按「另有币无催化剂」「另有币未关联资产」「另有币无催化剂、未关联资产」"
