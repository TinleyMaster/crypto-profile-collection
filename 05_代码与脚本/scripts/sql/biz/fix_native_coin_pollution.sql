-- ============================================================
-- 修复 Bitcoin 等原生链币种的合�?+ source_map 污染
--
-- 问题�?--   1. BTC (asset_type='coin') 被绑定了 6 条非原生链合约（WBTC/包装�?桥接币）
--   2. BTC �?asset_source_map �?11 �?CG 映射（同名币/仿盘/包装币）�?--      全部 is_primary=False，正确的 bitcoin 也未标记�?primary
--   3. core.chain 表为空，不依赖它，直接用 chain text 字段操作
--
-- 修复范围：所�?asset_type='coin' 的原生币
-- ============================================================

BEGIN;

-- ========== 第一步：清理合约污染 ==========
-- �?asset_type='coin' 的资产，删除所�?is_native=False 的合�?-- （原生币不应该有非原生链的智能合约地址�?
-- 先统�?SELECT COUNT(*) AS contracts_to_delete
...
-- �?BTC (asset_id=2)，只保留 coin_id='bitcoin' �?CG 映射�?-- 其余同名/仿盘/包装币全部删除，并将 bitcoin 设为 primary
...
-- 删除�?bitcoin �?CG 映射（只保留真正�?Bitcoin�?DELETE FROM core.asset_source_map
WHERE asset_id = 2
  AND source_code = 'cg'
...
-- 找出所�?asset_type='coin' 且有多个 CG 映射的资产，�?symbol 匹配正确�?coin_id
