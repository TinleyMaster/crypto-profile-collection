...
来源：WorkBuddy 侧 `.workbuddy/memory/2026-09-26.md` 记「`351d2ae` 改动共享函数 `_fmt_price`（去尾零），**是否接受需用户拍板**（波及面超出传导逻辑 Scope）」。本轮用户拍板 = **核验 + 钉边界**（不含修改渲染行为）。
...
- **关键发现：该文件存在两个同名 `_fmt_price`**，分属不同邮件通道且**行为不同**：
  - **局部版**（`notifier.py:821`，`_build_fast_alert_html` = **盘面快告警**通道）：**不去尾零**、阈值档位不同（`≥1` 用 `.2f`）、**带 `$` 前缀**。渲染实测：`620.50 → "$620.50"`、`100.0 → "$100.00"`、`1234.0 → "$1,234.00"`（尾零噪音在此通道**仍存在**）。
...
- **探针 45 → 55 断言**：`workbench/test_major_event_alert.py` 新增第 10 节（去尾零纯度 4 项 + 模块级矩阵 5 项 + 科学计数陷阱守卫 1 项）。**回归零失败**：`test_fast_alert_audit_rest`(61/0)、`test_fast_alert_ai_veto`(46/0)。
- **未做 / 边界**：① 局部版（盘面快告警）仍不去尾零、精度口径与模块级不同 —— 是否统一未获授权；② 未改任何渲染行为，本轮仅核验 + 钉边界。
